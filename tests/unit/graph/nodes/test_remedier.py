...
def test_happy_path_returns_fixed_code(mock_get_llm):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT)
    mock_get_llm.return_value = mock_llm

    state = _make_state(cited_rules=["MISRA-21.3"], retrieved_rules=[_make_rule("MISRA-21.3")])
    result = remediate_code(state)
...
@patch("app.graph.nodes.remedier.get_llm")
def test_returns_correct_state_keys(mock_get_llm):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT)
    mock_get_llm.return_value = mock_llm

    result = remediate_code(_make_state())
...
@patch("app.graph.nodes.remedier.get_llm")
def test_json_parse_failure_returns_original_code(mock_get_llm):
    mock_llm = MagicMock()
    ...
    code = "int x = 1;"
    result = remediate_code(_make_state(code_snippet=code))
...
@patch("app.graph.nodes.remedier.get_llm")
def test_json_parse_key_error_returns_original_code(mock_get_llm):
    """parse_json_response succeeds but result dict is missing expected keys — should not crash."""
    mock_llm = MagicMock()
    # Valid JSON but with wrong keys triggers .get() returning "" — this is a graceful path, not an error
    # Test KeyError by patching parse_json_response to raise it
    mock_llm.invoke.return_value = _mock_llm_response({"unexpected": "keys"})
    mock_get_llm.return_value = mock_llm
...
@patch("app.graph.nodes.remedier.get_llm")
def test_parse_json_raises_value_error_triggers_fallback(mock_get_llm):
    mock_llm = MagicMock()
    mock_get_llm.return_value = mock_llm
    ...
    with patch("app.graph.nodes.remedier.parse_json_response", side_effect=ValueError("bad json")):
        ...
        result = remediate_code(_make_state(code_snippet="int z;"))
...
@patch("app.graph.nodes.remedier.get_llm")
def test_only_cited_rules_appear_in_llm_context(mock_get_llm):
    ...
    remediate_code(state)

    call_args = mock_llm.invoke.call_args[0][0]
    human_content = next(m.content for m in call_args if hasattr(m, "content") and "Cited rule" in m.content or "Uncited rule" in m.content)
    assert "Cited rule" in human_content
    assert "Uncited rule" not in human_content


@patch("app.graph.nodes.remedier.get_llm")
def test_fallback_to_all_rules_when_no_cited_match(mock_get_llm):
    ...
    call_args = mock_llm.invoke.call_args[0][0]
    human_content = next(m.content for m in call_args if "Fallback rule" in m.content or "No rule details" in m.content)
    assert "Fallback rule" in human_content


@patch("app.graph.nodes.remedier.get_llm")
def test_no_retrieved_rules_uses_default_message(mock_get_llm):
    ...
    call_args = mock_llm.invoke.call_args[0][0]
    human_content = next(m.content for m in call_args if "No rule details available" in m.content)
    assert "No rule details available." in human_content


@patch("app.graph.nodes.remedier.get_llm")
def test_empty_state_does_not_crash(mock_get_llm):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT)
    mock_get_llm.return_value = mock_llm
...
@patch("app.graph.nodes.remedier.get_llm")
def test_token_counts_from_usage_metadata(mock_get_llm):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT, input_tokens=200, output_tokens=80)
    mock_get_llm.return_value = mock_llm
...
@patch("app.graph.nodes.remedier.get_llm")
def test_missing_usage_metadata_defaults_to_zero(mock_get_llm):
    mock_llm = MagicMock()
...
@patch("app.graph.nodes.remedier.get_llm")
def test_llm_called_with_temperature_02(mock_get_llm):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT)
    mock_get_llm.return_value = mock_llm

    remediate_code(_make_state())

    mock_get_llm.assert_called_once_with(temperature=0.2)


@patch("app.graph.nodes.remedier.get_llm")
def test_llm_invoked_with_system_and_human_messages(mock_get_llm):
...
@patch("app.graph.nodes.remedier.get_llm")
def test_human_message_contains_code_snippet(mock_get_llm):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT)
    mock_get_llm.return_value = mock_llm

    code = "volatile int *ptr = (int*)0xDEAD;"
    remediate_code(_make_state(code_snippet=code))
...
@patch("app.graph.nodes.remedier.get_llm")
def test_human_message_contains_validation_result(mock_get_llm):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT)
    mock_get_llm.return_value = mock_llm

    validation = "Violated Rule 11.3: cast from int to pointer."
    remediate_code(_make_state(validation_result=validation))
...
@patch("app.graph.nodes.remedier.get_llm")
def test_estimated_cost_uses_calculate_gemini_cost(mock_get_llm, mock_cost):
    mock_llm = MagicMock()
    mock_llm.invoke.return_value = _mock_llm_response(VALID_RESULT, input_tokens=100, output_tokens=50)
    mock_get_llm.return_value = mock_llm
